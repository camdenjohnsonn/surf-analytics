            ...
            for b in BREAKS:
                cur.execute("""
                    UPDATE breaks SET
                        ...
                        lng                   = %s,
                        break_depth_m         = %s,
                        height_gate_factor    = %s,
                        ideal_tide_min_ft     = %s,
                        ideal_tide_max_ft     = %s,
                        ideal_period_min_s    = %s,
                        ideal_wind_dir_min    = %s,
                        ideal_wind_dir_max    = %s,
                        ideal_wind_max_kts    = %s,
                        ideal_swell_dir_min   = %s,
                        ideal_swell_dir_max   = %s,
                        min_wave_height_ft    = %s,
                        max_wave_height_ft    = %s,
                        beach_orientation_deg = %s,
                        primary_buoy          = %s,
                        tide_station          = %s
                    WHERE break_id = %s
                """, (
                    b["country"], b["break_type"], b["lat"], b["lng"],
                    b["break_depth_m"], b["height_gate_factor"],
                    b["ideal_tide_min_ft"], b["ideal_tide_max_ft"], b["ideal_period_min_s"],
                    b["ideal_wind_dir_min"], b["ideal_wind_dir_max"], b["ideal_wind_max_kts"],
                    b["ideal_swell_dir_min"], b["ideal_swell_dir_max"],
                    b["min_wave_height_ft"], b["max_wave_height_ft"],
                    b["beach_orientation_deg"], b["primary_buoy"], b["tide_station"],
                    b["break_id"],
                ))

    print("Updated 16 breaks with full metadata")

    with con.cursor() as cur:
        cur.execute("SELECT break_id, name, break_type, lat, lng FROM breaks ORDER BY break_id")
        rows = cur.fetchall()
        print(f"{'break_id':<10} {'name':<20} {'break_type':<10} {'lat':<12} {'lng'}")
        for r in rows:
            print(f"{r[0]:<10} {r[1]:<20} {r[2]:<10} {r[3]:<12} {r[4]}")
finally:
    con.close()
